...
predictprobgeo_plot<-function(sformula, 
                           plotname,
                           includenas=TRUE, 
                           type="ctt",
                           logit=FALSE, 
  ...
  ndata<-with(database, data.frame(ethnic=as.numeric(as.character(factor(rep(0:1, each =6)))), 
                                   soc = as.factor(rep(3,12)),
                                   av_cent=rep(mean(av_cent), 12),
                                   absdif_cent=rep(mean(absdif_cent), 12), 
                                   geodis=rep(seq(from = 0, to = 100, length.out = 6),2), 
                                   EARLIEST_FILING_YEAR=rep(as.factor(2000),12))
  )
  ...
  tikzDevice::tikz(file = paste0("../output/predicted_prob/geo_", plotname, confidence, ".tex"), width = 5.5, height = 2)
  
  measure.plot<-ggplot(ndata_, aes(x = geodis, y = PredictedProb)) + 
    scale_color_aaas() + theme_bw()  +
    # geom_ribbon(aes(ymin = LL,
    #                 ymax = UL, fill = ethnic), alpha = 1) +
    geom_line(aes(colour = ethnic), size = 0.5) + scale_x_continuous(breaks=c(0,10,20,30,40,50)) +
    theme(legend.title=element_blank()) +ylab("Predicted Prob.") + xlab("Geographic distance")
